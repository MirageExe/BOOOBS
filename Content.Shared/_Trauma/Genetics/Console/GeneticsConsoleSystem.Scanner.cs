...
public sealed partial class GeneticsConsoleSystem
{
    ...
    private void OnScanDoAfter(Entity<GeneticsScannerComponent> ent, ref ScanDoAfterEvent args)
    {
        ...
        _adminLog.Add(LogType.Action, LogImpact.Low, $"{ToPrettyString(mob)} was scanned by {ToPrettyString(args.User)} with console {ToPrettyString(ent)}");
...
[ByRefEvent]
public record struct ScannerConnectedEvent(EntityUid Scanner);

[ByRefEvent]
public record struct ScannerDisconnectedEvent;

[ByRefEvent]
public record struct ScannerInsertedEvent(EntityUid Target);

[ByRefEvent]
public record struct ScannerEjectedEvent;
