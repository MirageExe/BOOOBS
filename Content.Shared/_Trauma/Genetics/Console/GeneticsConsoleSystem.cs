...
public sealed partial class GeneticsConsoleSystem : EntitySystem
{
    ...
    private void OnScramble(Entity<GeneticsConsoleComponent> ent, ref GeneticsConsoleScrambleMessage args)
    {
        ...
        _adminLog.Add(LogType.Action, LogImpact.High, $"Scrambled genome of {ToPrettyString(mob)} by {ToPrettyString(args.Actor)} using console {ToPrettyString(ent)}");
    ...
    private void OnResetSequence(Entity<GeneticsConsoleComponent> ent, ref GeneticsConsoleResetSequenceMessage args)
    {
        ...
        _adminLog.Add(LogType.Action, LogImpact.Low, $"{ToPrettyString(mob)} sequence {args.Index} was reset by {ToPrettyString(args.Actor)} using console {ToPrettyString(ent)}");
    ...
    private void OnWriteMutation(Entity<GeneticsConsoleComponent> ent, ref GeneticsConsoleWriteMutationMessage args)
    {
        ...
        _adminLog.Add(LogType.Action, LogImpact.Low, $"{mutation} from {ToPrettyString(mob)} was written to {ToPrettyString(disk)} by {ToPrettyString(args.Actor)} using console {ToPrettyString(ent)}");
    ...
    private void OnCombineDoAfter(Entity<GeneticsConsoleComponent> ent, ref CombineDoAfterEvent args)
    {
        ...
        _adminLog.Add(LogType.Action, LogImpact.Medium, $"{result} combined from {mutation} and {diskMutation} by {ToPrettyString(args.User)} using console {ToPrettyString(ent)}");
    ...
    private void OnPrint(Entity<GeneticsConsoleComponent> ent, ref GeneticsConsolePrintMessage args)
    {
        ...
        _adminLog.Add(LogType.Action, LogImpact.Medium, $"Printed {ToPrettyString(item)} with {mutation} by {ToPrettyString(user)} using console {ToPrettyString(ent)}");
    }
