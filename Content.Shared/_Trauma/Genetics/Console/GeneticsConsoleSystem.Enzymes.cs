...
public sealed partial class GeneticsConsoleSystem
{
    ...
    private void OnSaveEnzymes(Entity<GeneticsConsoleEnzymesComponent> ent, ref GeneticsConsoleSaveEnzymesMessage args)
    {
        ...
        _adminLog.Add(LogType.Action, LogImpact.Low, $"{ToPrettyString(args.Actor)} saved {name}'s unique enzymes to {ToPrettyString(disk)} with console {ToPrettyString(ent)}");
    ...
    private void OnApplyEnzymesDoAfter(Entity<GeneticsConsoleEnzymesComponent> ent, ref ApplyEnzymesDoAfterEvent args)
    {
        ...
        _adminLog.Add(LogType.Action, LogImpact.High, $"{ToPrettyString(args.User)} applied unique enzymes of '{enzymes.Name}' to {ToPrettyString(mob)} with console {ToPrettyString(ent)}");
